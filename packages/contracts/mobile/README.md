...
In the canonical product repository, `.github/workflows/sdk.yml` configures Node 22/24 Linux checks using PostgreSQL 17 and a macOS job for Swift, native HTTP and iOS simulator acceptance. The macOS job pins the reviewed Xcode installation, initializes its own temporary PostgreSQL cluster, and removes it at job end. Actions are pinned to reviewed commit SHAs; updates should be deliberate. The workflow reads no hosted credentials and performs no deployment or publication. Public SDK source mirrors run their separately documented standalone checks. Record actual hosted run results separately from local evidence; workflow presence alone does not prove execution. Runtime image/toolchain availability may require a reviewed update when GitHub retires an image.
